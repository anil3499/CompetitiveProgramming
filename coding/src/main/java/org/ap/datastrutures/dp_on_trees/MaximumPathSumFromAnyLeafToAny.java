...
//starting and ending node will be leaf node only
public class MaximumPathSumFromAnyLeafToAny {
    class Node {
        int value;
        Node left;
        Node right;
    ...
    public int max_path_sum(Node root, int res){
        ...
        int temp = max(l,r)+root.value);//calculate temp ans , doing 1 + to add node itself
        if (root.left==null && root.right==null) temp = max(temp,root.value);
    ...
    public static void main(String[] args) {
        Node root = new Node();
        int res = new MaximumPathSumFromAnyLeafToAny().max_path_sum(root,Integer.MIN_VALUE);
