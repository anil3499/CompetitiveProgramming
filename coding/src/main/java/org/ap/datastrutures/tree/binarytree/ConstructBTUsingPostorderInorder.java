...
public class ConstructBTUsingPostorderInorder {
    ...
    public static TreeNode<Integer> construct(int start, int end,
                                              int[] postorder, AtomicInteger postIndex,
                                              Map<Integer, Integer> map) {
        if(start>end){
            return null;
        }
        TreeNode root=new TreeNode(postorder[postIndex.getAndDecrement()]);

        int index=map.get(root.data);

        root.right=construct(index+1,end,postorder,postIndex,map);
        root.left=construct(start,index-1,postorder,postIndex,map);
