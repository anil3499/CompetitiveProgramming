...
public class EqualSumPartition {
    ...
    public static void main(String[] args) {
        ...
        if (sum %2 !=0){
            System.out.println("Cant have subste");
            return;
        }
        ...
        if (sum %2 ==0){
            int result = new EqualSumPartition().subsetsum(arr,sum/2,n);
            // we are doing divide /2 because if we get a subset whoe some is half of total sum ,
            // there must be an array whose sum is other half
            if ( result==1) {
